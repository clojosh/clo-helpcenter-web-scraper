...
scraped_html_path = os.path.join(project_path, "websites", "closet.com", "scraped_html")
openai_html_path = os.path.join(project_path, "websites", "closet.com", "openai_html")
...
class CLOSET:
    ...
    async def scrape_all_page_urls(self, page: Page, website: str):
        ...
        excluded_pages = [
            "https://style.clo-set.com/policy/cookie-policy",
            "https://style.clo-set.com/cookiesettings",
        ]
...
if __name__ == "__main__":
    ...
    if task in ["Scrape All URLs", "Scrape HTML Page", "Scrape All HTML Pages"]:
        ...
        if task == "Scrape All URLs":
            asyncio.run(web_scraper.playwright_scrape_all_page_urls("https://style.clo-set.com/"))

            if not os.path.exists(os.path.join(project_path, "websites", "closet.com")):
                os.makedirs(os.path.join(project_path, "websites", "closet.com"), exist_ok=True)

            with open(os.path.join(project_path, "websites", "closet.com", "scraped_urls.txt"), "w+") as f:
                for url in scraped_urls:
                    f.write(url + "\n")

        elif task == "Scrape HTML Page":
            urls = []
            if os.path.exists(os.path.join(project_path, "websites", "closet.com", "scraped_urls.txt")):
                with open(os.path.join(project_path, "websites", "closet.com", "scraped_urls.txt"), "r") as f:
